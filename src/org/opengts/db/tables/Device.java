    ...
    implements DataTransport
{
  ...
  //public static final String FLD_smsGatewayProps       = "smsGatewayProps";       // SMS gateway properties
    ...
    public static final String FLD_pendingMessage        = "pendingMessage";        // pending message to send to device
  ...
  //public static final String FLD_rentalAllowedKM       = "rentalAllowedKM";       // rental allowed kilometers
    ...
    **/
    public int getCurrentIgnitionState(boolean checkSC, boolean update)
    {
        ...
        double ignHours = this.getLastIgnitionOnHours();
    ...
    **/
    public boolean hasPendingMessage()
    {
        ...
        if (this.isPendingMessageAcknowledged()) {
            return false;
        } else {
            return true;
        }
    ...
    **/
    ...
    /**
    *** Sets the pending message acknowledgement for this device
    *** @param ack       The acknowledgement type ("%WAIT", "%SENT", "%RESP")
    *** @param timestamp The acknowledgement timestamp
    *** @param response  The acknowledgement response (if any)
    *** @param update    True to update the Device record now
    **/
    public boolean setPendingMessageACK(String ack, long timestamp, String response, boolean update)
    {
        // -- assemble and set ACK
        StringBuffer sb = new StringBuffer();
        sb.append(ack);
        sb.append(":");
        sb.append(timestamp);
        ...
        this.setPendingMessageACK(sb.toString());
        // -- update?
        if (update) {
            try {
                this.update(Device.FLD_pendingMessageACK);
                return true;
            } catch (DBException dbe) {
    ...
    /**
    *** Sets the pending message acknowledgement to "%WAIT" for this device,
    *** indicating that the pending message is waiting to be sent.
    *** @param timestamp The time the pending message was set to "%WAIT"
    *** @param update    True to update the Device record now
    **/
    public void setPendingMessageACK_WAIT(long timestamp, boolean update)
    {
        this.setPendingMessageACK("%WAIT", timestamp, null, update);
    }

    /**
    *** Sets the pending message acknowledgement to "%SENT" for this device,
    *** indicating that the pending message has been sent.
    *** @param timestamp The time the pending message was sent (used as MessageID)
    *** @param update    True to update the Device record now
    **/
    public void setPendingMessageACK_SENT(long timestamp, boolean update)
    {
        this.setPendingMessageACK("%SENT", timestamp, null, update);
    }

    /**
    *** Sets the pending message acknowledgement response for this device,
    *** indicating that the driver has responded to the pending message.
    *** @param timestamp The time the response was received
    *** @param response  The acknowledgement response (if any)
    *** @param update    True to update the Device record now
    **/
    public void setPendingMessageACK_RESP(long timestamp, String response, boolean update)
    {
        this.setPendingMessageACK("%RESP", timestamp, response, update);
    }

    /**
    *** Returns true if the pending message has been acknowledged
    *** @return True if the pending message has been acknowledged
    **/
    public boolean isPendingMessageAcknowledged()
    {
        String ack = this.getPendingMessageACK();
        if (StringTools.startsWithIgnoreCase(ack,"%RESP")) {
            return true;
        } else {
            return false;
        }
    ...
    /**
    *** Returns true if a pending message is waiting to be sent
    *** @return True if a pending message is waiting to be sent
    **/
    public boolean isPendingMessageWaiting()
    {
        String ackStr = this.getPendingMessageACK();
        if (StringTools.startsWithIgnoreCase(ackStr,"%WAIT")) {
            return true;
        } else {
            return false;
        }
    ...
    /**
    *** Returns true if a pending message has been sent
    *** @return True if a pending message has been sent
    **/
    public boolean isPendingMessageSent()
    {
        String ackStr = this.getPendingMessageACK();
        if (StringTools.startsWithIgnoreCase(ackStr,"%SENT")) {
            return true;
        } else {
            return false;
        }
    ...
    /**
    *** Gets the pending message acknowledge timestamp
    *** @return The pending message acknowledge timestamp
    **/
    public long getPendingMessageTimestamp()
    {
        String ackStr = this.getPendingMessageACK();
        String ack[] = StringTools.split(ackStr,':');
        return (ack.length > 1)? StringTools.parseLong(ack[1],0L) : 0L;
    }
    ...
    **/
    protected boolean _insertEventData(final EventData evdb)
    {
        ...
        */
        ...
        /* ignition state */
        double ignHours = this.getLastIgnitionOnHours(); // ignition-hours at ignition on
        long lastIgnOn  = this.getLastIgnitionOnTime();
        long lastIgnOff = this.getLastIgnitionOffTime();
        ...
        double engHours = this.getLastEngineOnHours(); // engine-hours at engine on
